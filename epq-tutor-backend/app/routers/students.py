...
def _to_summary(s: models.Student) -> StudentSummarySchema:
    ...
    return StudentSummarySchema(
        ...
        sessions=[SessionSummarySchema(id=x.id, type=x.type, date=x.date, time=x.time, durationMinutes=x.duration_minutes, feedbackSent=x.feedback_sent or False, isFinalDefense=x.is_final_defense or False, tutorAttending=x.tutor_attending or False) for x in s.sessions],
        availabilityNote=s.availability_note or "", briefNote=s.brief_note or "",
