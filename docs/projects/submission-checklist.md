...
| D6 | 「额外提交材料」= 临时项，**只留一个自由桶** | 每个学生要交的材料不同，成不了列。两个自由桶（额外材料 / 其他）会让人每次犹豫往哪放。 |
...
# students
submission_checklist      = Column(JSON)      # {"ticked": {itemId: "2026-08-29T10:00"}, "customItems": [{id,label,done,doneAt}]}
...
- `StudentSummarySchema` 新增：`submissionChecklist`、`tiiChecks`、`effectiveDeadline`、`deadlineTier`、`deadlineNeedsConfirm`、`wrappedUpAt`。
...
学生      ddl                表格 检测 录屏 额外   Tii        结项
张三      08.14 五 17:00     ✓    ✓    ·    ·      2/3 AI 11%  [结项]
          还剩 3天2h
李四 ⚠    08.21 五 17:00     ✓    ·    ·    1/3    0/3         [结项]
          延期·运营未确认      还剩 10天
王五      待定 ⚠             ·    ·    ·    ·      1/3         [结项]
──────────────────────────────────────────────
已结项 (2) ▸
...
- 列 = 当前模板项（活定义，模板改了列跟着变）+ 额外项计数列 + Tii 列 + 结项列
- **排序**：有效 ddl 升序 → 同 ddl 按未完成项数降序 → ddl 待定排最后 → 结项沉底折叠
- 点格子直接切换打钩（乐观更新 + 失败回滚），不进详情页
- `⚠` = 运营未确认；Tii ≥3 次标红
- 额外项列点开在行内展开，不做全局列（各学生不同，成不了列）
