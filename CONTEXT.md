...
**表13证据（per-student evidence list）.** Alongside the template items, each student carries a 表13证据 list stored in `students.submissionChecklist.customItems`. Its first two entries — 论文/报告 and PPT的PDF — are **fixed**: every student has them, they cannot be deleted, and the server re-adds them on both read and write (so pre-existing students need no migration). Everything after them is free-form, added and removed per student — every student hands in a different set, so those cannot be template columns. A single free bucket, not two: separate "额外材料" and "其他" buckets would leave the tutor guessing which one a given item belongs in.
