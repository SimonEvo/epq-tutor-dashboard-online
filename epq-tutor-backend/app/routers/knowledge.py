...
def kb_digest(
    ...
    prompt = (
        "你是一个EPQ教学助理。下面是本次与AI关于某学生的对话，以及尚未消化的原料碎片。"
        "请从中提炼出【值得长期保留的耐久事实】（几条即可，聚焦对未来辅导有用的判断/背景，"
        "剔除临时性、重复、已知的内容）。\n\n"
        f"## 未消化原料\n{entry_block}\n\n## 本次对话\n{convo}\n\n"
        "只输出一个 JSON 字符串数组，每个元素是一条耐久事实，不要输出其它任何内容。"
...
def merge_living_summary(
    ...
    prompt = (
        "你是一个EPQ教学助理，负责维护某学生的『活总结』——一份不断进化、代表当前对该学生理解的长文本。"
        "请把下列【新增耐久事实】自然地合并进现有活总结：保留原有有效信息，整合而非简单堆叠，"
        "去重、消解冲突（以新事实为准），保持条理清晰的 Markdown。\n\n"
        f"## 现有活总结\n{current}\n\n## 新增耐久事实\n{facts_block}\n\n"
